#!/bin/bash
###################################################################################
# Title         : enable-inspector-target-regions.sh
# Description   : 指定リージョンのAmazon Inspectorを有効化する。
# Author        : IIJ ytachiki
# Date          : 2025.04.01
###################################################################################
...
# ログファイル
LOGFILE=$(pwd)/enable-inspector-target-regions.log
...
function main(){
    ...
    # 指定リージョンでInspectorを有効化
    info 指定リージョンでInspectorを有効化
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    info '指定リージョンのAmazon Inspectorを有効化 開始'
    main $1 $2
    if [[ $? = 0 ]]; then
        info '指定リージョンのAmazon Inspectorを有効化 正常終了'
        exit 0
    else
        err '指定リージョンのAmazon Inspectorを有効化 異常終了'
        exit 1
    fi
